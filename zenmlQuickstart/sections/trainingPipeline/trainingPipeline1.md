...
Now that we have our data ready it makes sense to train some models to get a sense of
...
directly into a pipeline using the `Client().get_artifact_version` interface (`lines 56 and 57`). This is very convenient in this case, as we'd like to send our preprocessed dataset from the older pipeline directly into the training pipeline.
